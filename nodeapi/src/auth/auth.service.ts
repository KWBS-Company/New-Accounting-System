...
export class AuthService {
    ...
    async register(
        ...
        const result = await this.dataSource.transaction(async (manager) => {
            ...
            const currentFiscalYear = await manager.findOneBy(
                CustomerFiscalYear,
                {
            ...
            if (!currentFiscalYear) {
                const fiscalYrDates = this.commonService.getFiscalYearDates(
                    dto.fiscalStartDate,
                );
                ...
                const newFiscalstartDate =
                    this.commonService.getStartDateForNextFiscalYr(
                        currentFiscalYear.endDate,
                    );
                const fiscalYrDates =
                    this.commonService.getFiscalYearDates(newFiscalstartDate);
        ...
        await this.queueService.addEmailToQueue(
            result.user.email,
            'verify-email',
            { firstName: result.user.firstName, verificationUrl: result.url },
        );
    ...
    async login(dto: LoginDto): Promise<{
        ...
        if (user.userRoles.length === 0) {
            throw new UnauthorizedException(
                'User is not assigned with any roles',
            );
        }

        const isValid = await this.commonService.compareHash(
            dto.password,
            user.password,
        );
        if (!isValid) {
        ...
        return {
            accessToken,
            user: { ...safe, hasPassword: Boolean(password) },
        };
    }
    ...
    async resendVerification(email: string): Promise<{ message: string }> {
        ...
        // Usually keep external operations outside transaction
        await this.queueService.addEmailToQueue(user.email, 'verify-email', {
    ...
    async resetPassword(
        ...
        try {
            ...
            throw new BadRequestException(
                'Invalid or expired reset password Url token',
            );
        }
    ...
    async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
        ...
        await this.queueService.addEmailToQueue(user.email, 'reset-password', {
            firstName: user.firstName,
    ...
    async changePassword(user: User, changePasswordDto: ChangePasswordDto) {
        ...
        if (user.password) {
            try {
                checkPassword = await this.commonService.compareHash(
                    currentPassword,
                    user.password,
                );
            } catch (error) {
    ...
    async uploadProfilePicture(file: Express.Multer.File, user: User) {
        ...
        await this.usersService.update(user.id, {
            avatarUrl: `/uploads/profile-pic/${file.filename}`,
        });
        return {
