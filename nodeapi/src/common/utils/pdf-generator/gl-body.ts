...
function drawAccountInfoBox(
    ...
    page.drawText(typeLabel, {
        ...
        size: 8.5, font: bold, color: COLORS.darkGray,
    });
...
function drawOpeningRow(
    ...
    page.drawText("Opening Balance", {
        x: cols[0].x + 3, y: y - ROW_H + 7,
        size: 9, font: bold, color: COLORS.darkGray,
    });

    const balStr = formatAmount(openingBalance);
    const balW = bold.widthOfTextAtSize(balStr, 9);
    page.drawText(balStr, {
        x: cols[5].x + cols[5].w - balW - 3, y: y - ROW_H + 7,
        size: 9, font: bold, color: COLORS.black,
    });
...
function drawLedgerRow(
    ...
    // Date
    page.drawText(truncate(formatDate(line.transactionDate), regular, SIZE, cols[0].w - 6), {
        x: cols[0].x + 3, y: y - ROW_H + 7, size: SIZE, font: regular, color: COLORS.bodyText,
    });

    // Voucher No
    const vnLabel = line.serialNumber ? `VN-${line.serialNumber}` : "-";
    page.drawText(truncate(vnLabel, regular, SIZE, cols[1].w - 6), {
        x: cols[1].x + 3, y: y - ROW_H + 7, size: SIZE, font: regular, color: COLORS.darkGray,
    });

    // Description
    page.drawText(truncate(line.description || "-", regular, SIZE, cols[2].w - 6), {
        x: cols[2].x + 3, y: y - ROW_H + 7, size: SIZE, font: regular, color: COLORS.bodyText,
    });
    ...
    page.drawText(debitStr, {
        x: cols[3].x + cols[3].w - debitW - 3, y: y - ROW_H + 7,
        size: SIZE, font: regular, color: debitColor,
    });
    ...
    page.drawText(creditStr, {
        x: cols[4].x + cols[4].w - creditW - 3, y: y - ROW_H + 7,
        size: SIZE, font: regular, color: creditColor,
    });

    // Running balance
    const balStr = formatAmount(runningBalance);
    const balColor = runningBalance < 0 ? COLORS.red : COLORS.black;
    const balW = regular.widthOfTextAtSize(balStr, SIZE);
    page.drawText(balStr, {
        x: cols[5].x + cols[5].w - balW - 3, y: y - ROW_H + 7,
        size: SIZE, font: regular, color: balColor,
    });
}
...
function drawClosingRow(
    ...
    page.drawText("Closing Balance", {
        x: margin + 3, y: y + 7, size: SIZE, font: bold, color: COLORS.black,
    });
    ...
    page.drawText(tdStr, {
        x: cols[3].x + cols[3].w - tdW - 3, y: y + 7, size: SIZE, font: bold, color: COLORS.black,
    });
    ...
    page.drawText(tcStr, {
        x: cols[4].x + cols[4].w - tcW - 3, y: y + 7, size: SIZE, font: bold, color: COLORS.black,
    });
    ...
    page.drawText(cbStr, {
        x: cols[5].x + cols[5].w - cbW - 3, y: y + 7, size: SIZE, font: bold, color: cbColor,
    });
...
export function drawLedgerBody(
    ...
    curPage.drawText(periodStr, {
        x: margin + contentW - psW, y, size: META_SIZE, font: regular, color: COLORS.darkGray,
    });
    y -= 18;
    ...
    for (const line of data.account.lines) {
        ...
        drawLedgerRow(curPage, ctx, cols, line, runningBalance, y);
        y -= ROW_H;
    }
    ...
    drawClosingRow(curPage, ctx, cols, totalDebit, totalCredit, runningBalance, y);
