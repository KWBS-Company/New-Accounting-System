...
function buildPLColumns(
  ...
  const defs: Omit<PLColDef, "w" | "x">[] = [
    { label: sectionLabel,          key: "code",        widthFraction: 0.14, align: "left"  },
    { label: "",                    key: "name",        widthFraction: 0.56, align: "left"  },
    { label: "",                    key: "accountType", widthFraction: 0.14, align: "left"  },
    { label: `Amount (${currency})`,key: "balance",     widthFraction: 0.16, align: "right" },
  ];
...
function drawLineItems(
  ...
  for (const item of items) {
    ...
    if (y - ROW_H < layout.margin + FOOTER_SPACE) {
      ...
      const newCols = buildPLColumns(sectionLabel, currency, contentW, margin);
    ...
    // accountType (col 2)
    const typeVal = truncate(item.accountType ?? "", regular, 9, cols[2].w - 8);
...
export function drawPLBody(
  ...
  const revResult = drawLineItems(
    pdfDoc, curPage, ctx, revCols, data.revenues, y, "Revenue", currency,
  );
  curPage = revResult.page;
  y       = revResult.y;

  y = drawSubtotalRow(curPage, ctx, "Total Revenue", data.summary.totalRevenue, revCols, y);
  ...
  const expResult = drawLineItems(
    pdfDoc, curPage, ctx, expCols, data.expenses, y, "Expenses", currency,
  );
  curPage = expResult.page;
  y       = expResult.y;

  y = drawSubtotalRow(curPage, ctx, "Total Expenses", data.summary.totalExpense, expCols, y);
