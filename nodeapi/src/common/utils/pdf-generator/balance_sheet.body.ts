...
import {
    BalanceSheetPDFData,
    BSLineItem,
} from 'src/accounts/types/balance_sheet.types';
// ─────────────────────────────────────────────────────────────────────────────
//  Constants
// ─────────────────────────────────────────────────────────────────────────────
...
function drawLineItems(
    ...
    for (const item of items) {
        ...
        // accountType
        page.drawText(
            truncate(item.accountType ?? '', regular, 9, cols[2].w - 8),
            {
                x: cols[2].x + 4,
                y: y - ROW_H + 7,
                size: 9,
...
function drawSummaryTable(
    ...
    y = summaryRow(
        'Total Liabilities',
        formatAmount(data.summary.totalLiabilities),
        false,
        y,
    );
    y = summaryRow(
        'Total Equity',
        formatAmount(data.summary.totalEquity),
        false,
        y,
    );
...
export function drawBSBody(
    ...
    drawSection(
        ...
        'Equity',
        'Total Equity',
        data.summary.totalEquity,
    );

    drawSection(
        [],
        'Profit',
        `${data.summary.currentYearNetPL > 0 ? 'Current Year Net Profit' : 'Current Year Net Loss'}`,
        data.summary.currentYearNetPL,
    );
