...
import {
    JournalLine,
    JournalVoucherData,
} from 'src/accounts/types/journal_voucher.types';
...
function buildCols(currency: string, contentW: number, marginX: number): Col[] {
    const fractions = [0.26, 0.34, 0.2, 0.2];
    const labels = [
        'Account',
        'Description',
        `Debit (${currency})`,
...
function drawMetaGrid(
    ...
    for (let i = 0; i < rows.length; i += 2) {
        ...
        if (rightRow) {
            const lW = regular.widthOfTextAtSize(
                rightRow.label + ' ',
                LABEL_SZ,
            );
