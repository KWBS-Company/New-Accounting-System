...
export const profitLossDataMapper = (
    ...
    const totalRevenue = dataWithBalance
        .filter((x) => x.accountType === AccountType.REVENUE)
        .reduce((sum, x) => sum + Number(x.balance), 0);
    const totalExpense = dataWithBalance
        .filter((x) => x.accountType === AccountType.EXPENSE)
        .reduce((sum, x) => sum + Number(x.balance), 0);
...
export const PLPdfDataMapper = (
    user: User,
    backendUrl: string,
    pl: ProfitLossData,
) => {
    const company = user.userRoles[0].customer;
    const revenues = pl.items.filter(
        (it) => it.accountType === AccountType.REVENUE,
    );
    const expenses = pl.items.filter(
        (it) => it.accountType === AccountType.EXPENSE,
    );
    ...
    const data: ProfitLossPDFData = {
        ...
        summary: {
            ...pl.summary,
            netProfitAbs: Math.abs(pl.summary.netProfit),
        },
