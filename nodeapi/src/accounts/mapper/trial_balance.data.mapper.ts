...
import {
    ...
    TrialBalanceItem,
    TrialBalancePDFData,
} from '../types/trial_balance.types';
import { User } from 'src/auth/entities/user.entity';
import { BadRequestException } from '@nestjs/common';

export const trialBalanceDataMapper = (
    ...
    const totalDebit = dataWithBalance.reduce(
        (prev, curr) => prev + Number(curr.debit),
        0,
    );
    const totalCredit = dataWithBalance.reduce(
        (prev, curr) => prev + Number(curr.credit),
        0,
    );
    return { items: dataWithBalance, summary: { totalCredit, totalDebit } };
};

export const trialBalancePdfDataMapper = (
    user: User,
    backendUrl: string,
    ...
    const data: TrialBalancePDFData = {
        ...
        accounts: trialBalance.items.map((tb) => ({
            ...tb,
        ...
        isMatched:
            trialBalance.summary.totalDebit ===
            trialBalance.summary.totalCredit,
    };
