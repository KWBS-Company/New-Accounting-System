...
export class TransactionService {
    ...
    private buildLine(params: {
        ...
        const increasesWithDebit = this.debitIncreaseTypes.includes(
            account.accountType,
        );
    ...
    async create(data: CreateTransactionDto, user: User) {
        const { reference, amount, lines, transactionDate } = data;
        const customerId = user.userRoles[0].customerId;
        const currentFiscalYr = user.userRoles[0].customer.fiscalYears.find(
        ...
        const isValid = this.commonService.isWithinFiscalYear(
            new Date(transactionDate),
            currentFiscalYr.startDate,
            currentFiscalYr.endDate,
        );

        if (!isValid) {
            throw new BadRequestException(
                `Transaction cannot be created since the transaction date is beyond current fiscal yr ${currentFiscalYr.name}`,
            );
        }
        this.validateBalance(lines);
        await this.dataSource.transaction(async (manager) => {
            ...
            for (const line of lines) {
                ...
                await manager.save(TransactionLine, transactionLine);
            }
        });
    ...
    async findById(id: string, user: User) {
        ...
        const data = await this.txnRepository.findOne({
            where: { id: id, deletedAt: IsNull(), customerId: customerId },
    ...
    async delete(id: string, user: User) {
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            const checkCurrentFiscalYrData = await manager.findOne(
                Transaction,
                {
                    where: {
                        id: id,
            ...
            if (!checkCurrentFiscalYrData) {
                throw new BadRequestException(
                    'Transaction you are trying to delete, not in current fiscal yr',
                );
            }
            ...
            for (const line of txn.lines) {
                ...
                await manager.save(TransactionLine, line);
            }
        });
    ...
    async listTransactionsWithPagination(
        ...
        const qb = this.txnRepository
            ...
            .leftJoinAndSelect(
                'txn.fiscalYear',
                'fy',
                'fy.deletedAt IS NULL AND fy.customerId = :customerId',
                { customerId },
            )
            .where('txn.deletedAt IS NULL AND txn.customerId = :customerId', {
                customerId,
            })
        ...
        if (!query.fiscalYearId) {
            ...
            qb.andWhere(`txn.fiscal_year_id = :currentFiscalYearId`, {
                currentFiscalYearId: query.fiscalYearId,
            });
        }
        ...
        if (query.transactionFrom) {
            qb.andWhere('txn.transaction_date::date >= :transactionFrom', {
        ...
        if (query.transactionTo) {
            qb.andWhere('txn.transaction_date::date <= :transactionTo', {
    ...
    async update(id: string, data: UpdateTransactionDto, user: User) {
        const { reference, amount, transactionDate, lines } = data;
        ...
        if (!isValid) {
            throw new BadRequestException(
                `Transaction cannot be updated since the transaction date is beyond current fiscal yr ${currentFiscalYr.name}`,
            );
        }
        this.validateBalance(lines);
        await this.dataSource.transaction(async (manager) => {
            ...
            if (!existingTransaction) {
                throw new BadRequestException(
                    'Transaction not found to update.',
                );
            }
            ...
            if (!checkCurrentFiscalYrData) {
                throw new BadRequestException(
                    'Transaction you are trying to get, not in current fiscal yr',
                );
            }
            ...
            for (const line of lines) {
                const transactionLine = await manager.findOne(TransactionLine, {
                    where: {
                        deletedAt: IsNull(),
                        id: line.lineId,
                    },
                ...
                if (transactionLine) {
                    await manager.update(TransactionLine, line.lineId, {
                        debit: line.debit,
                        credit: line.credit,
                        description: line.description,
                        accountId: line.accountId,
                    ...
                    await manager.save(TransactionLine, newTransactionLine);
                }
            }
        });
    ...
    async uploadExcel(file: Express.Multer.File, user: User) {
        const customerId = user.userRoles[0].customerId;
        ...
        try {
            const excelData =
                await this.accountExcelService.getTransactionTemplateData(file);

            for (const data of excelData) {
                const txnType = await queryRunner.manager.findOne(
                    TransactionType,
                    {
                        where: {
                ...
                const transactionLines = await this.generateJournalLines({
                    rules: txnType.rules,
                ...
                for (const line of transactionLines) {
                    const transactionLine = queryRunner.manager.create(
                        TransactionLine,
                        {
    ...
    async downloadTransactionTemplate(user: User) {
        const customerId = user.userRoles[0].customerId;

        const transactionTypes = await this.txnTypeRepository.find({
            where: {
                deletedAt: IsNull(),
                customerId,
            },
        ...
        return await this.accountExcelService.generateTransactionTemplate(
            transactionTypes,
        );
    }

    async downloadJournalVoucher(transactionId: string, user: User) {
        const backendUrl =
        ...
        const pdfBuffer =
            await this.accountPdfService.journalVoucherPdfGenerator(
                pdfMappedData,
            );
        return pdfBuffer;
    }
    ...
    async previewTransactionLine(dto: PreviewTransactionLineDto, user: User) {
        ...
        const txnType = await this.txnTypeRepository.findOne({
            where: {
