...
export class AccountMCPService {
    ...
    async getAccountDetailByKeyAndFilter(dto: MCPDataDto) {
        ...
        if (dto.key.toLocaleLowerCase().includes('id')) {
            ...
            return await this.accountRepository.findOne({ where: { deletedAt: IsNull(), id: dto.value, customerId }, relations: ['children'] });

        } else if (dto.key.toLocaleLowerCase().includes('code')) {
            return await this.accountRepository.findOne({ where: { deletedAt: IsNull(), code: ILike(dto.value), customerId }, relations: ['children'] });

        }
        else if (dto.key.toLocaleLowerCase().includes('name')) {
            return await this.accountRepository.findOne({ where: { deletedAt: IsNull(), name: ILike(dto.value), customerId }, relations: ['children'] });
    ...
    async listAccounts(dto: MCPDataDto) {
        const customerId = dto.customerId;
        return this.accountRepository.find({ where: { deletedAt: IsNull(), customerId }, relations: ['children'] });
    }
    ...
    async getBalance(dto: MCPDataDto) {
        ...
        if (dto.key.toLocaleLowerCase().includes('id')) {
            ...
            account = await this.accountRepository.findOne({
                where: {
                    deletedAt: IsNull(), id: dto.value, customerId,
                    lines: {
                        deletedAt: IsNull()
                    }
            ...
            account = await this.accountRepository.findOne({
                where: {
                    deletedAt: IsNull(), code: ILike(dto.value), customerId,
                    lines: {
                        deletedAt: IsNull()
                    }
                }, relations: ['children', 'lines']
            ...
            account = await this.accountRepository.findOne({
                where: {
                    deletedAt: IsNull(), name: ILike(dto.value), customerId, lines: {
                        deletedAt: IsNull()
                    }
                }, relations: ['children', 'lines']
            });
        ...
        const balance = account.lines.map(l => l.debit - l.credit).reduce((cur, prev) => cur + prev, 0);
