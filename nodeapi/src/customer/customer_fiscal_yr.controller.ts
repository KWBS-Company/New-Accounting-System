...
export class CustomerFiscalYearController {
    ...
    async listCustomerFiscalYears(@CurrentUser() user: User) {
        return await this.customerFiscalYearService.findAllByCustomerId(
            user.userRoles[0].customerId,
        );
    }

    @Patch('')
    async patchFiscalYear(@CurrentUser() user: User) {
        return await this.customerFiscalYearService.patchCurrentFiscalYear(
            user,
        );
    }
}
