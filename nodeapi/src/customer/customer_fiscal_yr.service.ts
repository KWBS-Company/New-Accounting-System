...
export class FiscalYearService {
    constructor(
        @InjectRepository(CustomerFiscalYear)
    ...
    async create(
        data: Partial<CustomerFiscalYear>,
    ): Promise<CustomerFiscalYear> {
    ...
    async patchCurrentFiscalYear(user: User) {
        ...
        await this.dataSource.transaction(async (manager) => {
            const currentFiscalYear = await manager.findOne(
                CustomerFiscalYear,
                {
                    where: {
                        deletedAt: IsNull(),
            ...
            if (!currentFiscalYear) {
                throw new BadRequestException(
                    'Fiscal year has been set up yet.',
                );
            }

            const transactionIds = currentFiscalYear.transactions.map(
                (t) => t.id,
            );
            const transactionLines = await manager.find(TransactionLine, {
                where: {
                    deletedAt: IsNull(),
            ...
            const incomes = transactionLines.filter(
                (tl) => tl.account.accountType === AccountType.REVENUE,
            );
            const liabilities = transactionLines.filter(
                (tl) => tl.account.accountType === AccountType.LIABILITY,
            );
            const expenses = transactionLines.filter(
                (tl) => tl.account.accountType === AccountType.EXPENSE,
            );
            const assets = transactionLines.filter(
                (tl) => tl.account.accountType === AccountType.ASSET,
            );
            const equities = transactionLines.filter(
                (tl) => tl.account.accountType === AccountType.EQUITY,
            );

            const dataWithBalancePL = [...incomes, ...expenses].map((row) => {
                let balance = 0;
            ...
            const totalRevenue = dataWithBalancePL
                .filter((x) => x.account.accountType === AccountType.REVENUE)
                .reduce((sum, x) => sum + Number(x.balance), 0);
            const totalExpense = dataWithBalancePL
                .filter((x) => x.account.accountType === AccountType.EXPENSE)
                .reduce((sum, x) => sum + Number(x.balance), 0);
            ...
            const dataWithBalanceBS = [
                ...
                if (
                    row.account.accountType === AccountType.LIABILITY ||
                    row.account.accountType === AccountType.EQUITY
                ) {
                    balance = Number(row.credit) - Number(row.debit);
                }
            ...
            const totalAssets = dataWithBalanceBS
                .filter((x) => x.account.accountType === AccountType.ASSET)
                .reduce((sum, x) => sum + Number(x.balance), 0);

            const totalLiabilities = dataWithBalanceBS
                .filter((x) => x.account.accountType === AccountType.LIABILITY)
                .reduce((sum, x) => sum + Number(x.balance), 0);

            const totalEquity = dataWithBalanceBS
                .filter((x) => x.account.accountType === AccountType.EQUITY)
                .reduce((sum, x) => sum + Number(x.balance), 0);
            ...
            if (totalLiabilitiesAndEquity !== totalAssets) {
                throw new BadRequestException(
                    'Cannot close fiscal yr since current liabilities and assets is not equal',
                );
            }

            const parentGeneralReserve = await manager.findOne(Account, {
                where: { deletedAt: IsNull(), customerId, code: 'GR0001' },
            });
            if (!parentGeneralReserve) {
                throw new BadRequestException(
                    'General Reserve account not found',
                );
            }
            let retAccount: Account;
            const currentYearEarningAccount = await manager.findOne(Account, {
                where: { deletedAt: IsNull(), customerId, code: 'CYE0001' },
            });
            ...
            if (netProfit > 0) {
                ...
                // dr=> CYE
                //  cr=> GR
                const retTransaction = await manager.save(
                    Transaction,
                    newTransaction,
                );
                const CYELine = manager.create(TransactionLine, {
                    transactionId: retTransaction.id,
                    debit: Math.abs(netProfit),
                    credit: 0,
                    description:
                        'Net profit transferred to General Reserve account',
                    accountId: retAccount.id,
                });
                const GA = manager.create(TransactionLine, {
                    transactionId: retTransaction.id,
                    debit: 0,
                    credit: Math.abs(netProfit),
                    description:
                        'Net profit transferred to General Reserve account',
                    accountId: parentGeneralReserve.id,
                });
                ...
                const CYELine = manager.create(TransactionLine, {
                    ...
                    credit: Math.abs(netProfit),
                    description:
                        'Net loss transferred to General Reserve account',
                    accountId: retAccount.id,
                });
                const GA = manager.create(TransactionLine, {
                    transactionId: retTransaction.id,
            ...
            if (currentDate < fiscalYrEndDate) {
                throw new BadRequestException(
                    'Cannot closed current fiscal yr since end date is not crossed',
                );
            }
            const newStartDate =
                this.commonService.getStartDateForNextFiscalYr(fiscalYrEndDate);

            await manager.update(CustomerFiscalYear, currentFiscalYear.id, {
            ...
            const { startDate, endDate, name } =
                this.commonService.getFiscalYearDates(newStartDate);

            const newFY = manager.create(CustomerFiscalYear, {
                customerId,
                name,
                startDate,
                endDate,
