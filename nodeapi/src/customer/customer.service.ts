...
export class CustomerService {
    ...
    async listCustomers(query: ListCustomerQuery) {
        ...
        qb.andWhere(
            `customer.deletedAt IS NULL AND customer.companyName <> 'MASTER'`,
        );
        qb.orderBy('customer.updatedAt', 'DESC', 'NULLS LAST');

        if (searchQuery) {
            qb.andWhere(
                '( customer.companyName ILIKE :search OR customer.companyEmail ILIKE :search OR customer.companyPhone ILIKE :search OR customer.companyAddress ILIKE :search )',
                { search: `%${searchQuery}%` },
            );
        }
    ...
    async updateCustomer(
        ...
        if (roleType !== RoleType.SUPER_ADMIN && customerId !== id) {
            throw new ForbiddenException(
                'Cannot perform action due to lack of privilages',
            );
        }
        const {
            companyName,
    ...
    async uploadCompanyLogo(file: Express.Multer.File, id: string, user: User) {
        const roleType = user.userRoles[0].roleType;
        const customerId = user.userRoles[0].customerId;
        if (roleType !== RoleType.SUPER_ADMIN && customerId !== id) {
            throw new ForbiddenException(
        ...
        await this.update(id, {
            companyLogo: `/uploads/logo/${file.filename}`,
        });
        return {
