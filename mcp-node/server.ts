...
app.post("/test-chat", async (req, res) => {
  ...
  try {
    const { messages = [], model = "qwen2.5:14b", customerId = '44167d29-bf22-4996-aba0-65b10f159ee4' } = req.body;
    ...
    const userInfo = JSON.stringify({ customerId });
    ...
    if (toolCalls.length > 0) {
      ...
      for (const toolCall of toolCalls) {
        const result = await client.callTool({
          ...
          arguments: {
            ...toolCall.function.arguments, // whatever the LLM decided (key, value, showChild, etc.)
            customerId, // server-bound value always wins, overwriting any LLM-supplied customerId
          },
        });
